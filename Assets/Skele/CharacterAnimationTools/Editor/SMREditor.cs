...
public class SMREditor : Editor
{
    ...
    // LimbIK Constraints settings by Joints
    private static LimbConDict ms_DictTr2Constraint;
    private static ESetConstraintStep ms_SetConsStep = ESetConstraintStep.None;
    private static ESetConstraintStep ms_SetConsStep_Req = ESetConstraintStep.None;
    private static ELimbType ms_eLimbType = ELimbType.None;
    ...
    // Skele_CRCont
    private static Skele_CRCont ms_crPaint; //paint event coroutine-container    
	#endregion "Misc"   
    ...
    static SMREditor()
    {
        ...
        ms_DictTr2Constraint = new LimbConDict();
    ...
    void OnSceneGUI()
    {
            ...
            case EventType.Layout:
                {
                    ...
                    if( ms_SetConsStep != ms_SetConsStep_Req )
                    {
                        _OnChangeSetConstraintStep(ms_SetConsStep, ms_SetConsStep_Req);
                    }
        ...
        else if( ms_eOpMode == EOPMode.SetConstraint ) //Route-D
        {
            switch( ms_SetConsStep )
            {
                case ESetConstraintStep.SelectLimbType:
                    {
                        _SceneRepaintAll();
                    }
                    break;
                case ESetConstraintStep.RotateMidJoint:
                    {
                        // draw Rotation_Handles at MidJoint
                        Transform[] joints = ms_IKSolver.GetJoints();
                        Transform midJoint = joints[1];

                        Quaternion q = Handles.RotationHandle(midJoint.rotation, midJoint.position);
                        if( GUI.changed )
                        {
                            Undo.RecordObject(midJoint, "Rotate_MidJoint");
                            midJoint.rotation = q;
                        }

                        //Draw bone and joint markers
                        _DrawMarkersForBones(joints[0], joints);
                    }
                    break;
                default:
                    Dbg.LogErr("SMREditor.OnSceneGUI: Route-D: unexpected ms_SetConsStep: {0}", ms_SetConsStep);
                    break;
            }
        }
    ...
    private void _OnStartEdit()
    {
        ...
        ms_BackupMat = ms_PS.renderer.sharedMaterial;
        _SetVertMaterial();
        ...
        var planeMat = ms_IKPlaneGO.renderer.material = new Material(Shader.Find("Sprites/Default"));
    ...
    private static void _OnEndEdit()
    {
        ...
        GameObject.DestroyImmediate(ms_IKPlaneGO.renderer.sharedMaterial); //clean the new-ed material
        ms_IKPlaneGO = null;
        ...
        ms_SetConsStep = ms_SetConsStep_Req = ESetConstraintStep.None;

        ms_eLimbType = ELimbType.None;

        ms_PinIKSolvers.Clear();

        ms_DictTr2Constraint.Clear();
            ...
            Handles.BeginGUI();
            {
                ...
                switch (ms_eOpMode)
                {
                    ...
                    case EOPMode.IK_Pinned:
                        ...
                        break;
                    case EOPMode.SetConstraint:
                        {
                            _DrawSceneGUI_SetConstraint();
                        }
                        break;
...
");
    ...
    private static void _DrawSceneGUI_IK()
    {
        ...
        // The Constraint Switch ( only work for 2-bone link )
        if( !ms_SelCtrl.NoSelection && ms_IKSolver.Count == 2)
        {
            Transform selJoint = ms_SelCtrl.SingleJoint;
            Rect cRect = new Rect(0, Screen.height - 60, 80, 20);
            GUILayout.BeginArea(cRect);
            {
                GUILayout.BeginHorizontal();
                {
                    bool bHasConstraint = ms_DictTr2Constraint.ContainsKey(selJoint);
                    ...
                    // toggle limb_constraint
                    if ( EUtil.Button("C", "Control_IK_Constraints", c))
                    {
                        Transform[] joints = ms_IKSolver.GetJoints();
                        ...
                        if( bHasConstraint )
                        {
                            ms_DictTr2Constraint.Remove(selJoint);
                            _TryMakeIKLink(endJoint, 2); //revert to BaseIKSolver
                            EUtil.ShowNotification("Removed_Limb_Constraints");
                        }
                        else
                        {
                            ms_eOpMode_Req = EOPMode.SetConstraint;
                            EUtil.ShowNotification("Start_Adding_Constraint");
                        }
                    } // if( EUtil.Button("C", ... )

                    // straighten the bones
                    if( EUtil.Button("S", "Straighten_the_Limb", Color.white))
                    {
                        EUtil.ShowNotification("Straighten_the_Limb");
                        Transform[] joints = ms_IKSolver.GetJoints();
                        Transform rootJoint = joints[0];
                        Transform midJoint = joints[1];
                        Transform endJoint = joints[joints.Length - 1];

                        Vector3 expectedDir = (midJoint.position - rootJoint.position).normalized;
                        Vector3 curDir = (endJoint.position - midJoint.position).normalized;

                        Undo.RecordObjects(joints, "Straighten Limb");
                        IKSolverUtil.JointLookAt(midJoint, expectedDir, curDir);
    ...
    private static void _DrawSceneGUI_SetConstraint()
    {
        if( ms_SetConsStep == ESetConstraintStep.SelectLimbType )
        {
            Rect rect = new Rect(Screen.width*0.5f - 50, Screen.height*0.5f - 60, 120, 120);
            if (ms_Background != null)
                GUI.DrawTexture(rect, ms_Background);

            GUILayout.BeginArea(rect);
            {
                EditorGUILayout.LabelField("Select Limb Type:");

                if (EUtil.Button("Arm_R", "Right Arm"))
                {
                    EUtil.ShowNotification("Right_Arm");
                    ms_SetConsStep_Req = ESetConstraintStep.RotateMidJoint;
                    ms_eLimbType = ELimbType.R_Arm;
                    _SceneRepaintAll();
                }
                if (EUtil.Button("Arm_L", "Left Arm"))
                {
                    EUtil.ShowNotification("Left_Arm");
                    ms_SetConsStep_Req = ESetConstraintStep.RotateMidJoint;
                    ms_eLimbType = ELimbType.L_Arm;
                    _SceneRepaintAll();
                }
                if (EUtil.Button("Leg_R", "Right Leg"))
                {
                    EUtil.ShowNotification("Right_Leg");
                    ms_SetConsStep_Req = ESetConstraintStep.RotateMidJoint;
                    ms_eLimbType = ELimbType.R_Leg;
                    _SceneRepaintAll();
                }
                if (EUtil.Button("Leg_L", "Left Leg"))
                {
                    EUtil.ShowNotification("Left_Leg");
                    ms_SetConsStep_Req = ESetConstraintStep.RotateMidJoint;
                    ms_eLimbType = ELimbType.L_Leg;
                    _SceneRepaintAll();
                }
            }
            GUILayout.EndArea();
        }
        else if( ms_SetConsStep == ESetConstraintStep.RotateMidJoint )
        {
            Rect rect = new Rect(0, Screen.height - 60, 150, 40);
            ...
            GUILayout.BeginArea(rect);
            {
                GUILayout.BeginHorizontal();
                {
                    ...
                    if (EUtil.Button("Cancel", "Go Back to IK Mode", c))
                    {
                        EUtil.ShowNotification("Cancel_Setting_Constraint");
                        ms_eOpMode_Req = EOPMode.IK;
                        _SceneRepaintAll();
                    }

                    if( EUtil.Button("Done", "Confirm Setting", Color.green))
                    {

                        Transform[] joints = ms_IKSolver.GetJoints();
                        Transform skeleRoot = ms_ExtendedJoints[0];
                        Transform ikRootJoint = joints[0];
                        Transform midJoint = joints[1];
                        Transform endJoint = joints[joints.Length-1];

                        //decide the axis first 
                        Vector3 upper = midJoint.position - ikRootJoint.position;
                        Vector3 lower = endJoint.position - midJoint.position;
                        Vector3 worldRotAxis = Vector3.Cross(lower, upper).normalized; //rotate around the axis( from bend to straight )
                        if (worldRotAxis == Vector3.zero)
                        {
                            EUtil.ShowNotification("The_limb_is_STRAIGHT, BEND_it_first");
                        }
                        else
                        {
                            ms_eOpMode_Req = EOPMode.IK;

                            EUtil.ShowNotification("Confirmed_Constraint_Setting");
                            ConstraintInfo newInfo = new ConstraintInfo();

                            Vector3 localRotAxis = ikRootJoint.InverseTransformDirection(worldRotAxis); //  world to local conversion
                            //Dbg.Log("worldRotAxis: {0}, localRotAxis: {1}", worldRotAxis, localRotAxis);
                            
                            Vector3 ikRootRelRefDir = Vector3.zero;
                            Vector3 rootRelRefDir = Vector3.zero;
                            float angleThres = 0f;

                            switch( ms_eLimbType )
                            {
                                case ELimbType.L_Arm:
                                    {
                                        ikRootRelRefDir = -localRotAxis;
                                        rootRelRefDir = skeleRoot.InverseTransformDirection(-worldRotAxis);
                                        angleThres = 140f;
                                    }
                                    break;
                                case ELimbType.R_Arm:
                                    {
                                        ikRootRelRefDir = localRotAxis;
                                        rootRelRefDir = skeleRoot.InverseTransformDirection(worldRotAxis);
                                        angleThres = 140f;
                                ...
                                case ELimbType.R_Leg:
                                    ...
                                    break;
                                default:
                                    Dbg.LogErr("SMREditor._DrawSceneGUI_SetConstraint: unexpected LimbType: {0}", ms_eLimbType);
                                    break;
                            }

                            newInfo.m_SkeleRootJoint = skeleRoot;
                            newInfo.m_RotateAxis = localRotAxis;
                            newInfo.m_LimbRootRefDir = ikRootRelRefDir;
                            newInfo.m_SkeleRootRefDir = rootRelRefDir;
                            newInfo.m_AngleThres = angleThres;

                            ms_DictTr2Constraint[endJoint] = newInfo;
                            _TryMakeIKLink(endJoint, 2);
                        }
        ...
        {
            Dbg.LogErr("SMREditor._DrawSceneGUI_SetConstraint: unexpected enum: {0}", ms_SetConsStep);
        }
    }
            ...
            case EOPMode.SetConstraint:
                {
                    _PasteBackPose();
                    _ClearPoseBuffer();
                    ms_SetConsStep = ESetConstraintStep.None;
                    ms_SetConsStep_Req = ms_SetConsStep;
                    ms_eLimbType = ELimbType.None;
                }
                break;
            case EOPMode.PoseManage:
            ...
            case EOPMode.SetConstraint:
                {
                    ...
                    // revert to prefab pose, straighten specified limb, etc.
                    ms_crPaint.Start( _Job_PrepareSetConstraint() );

                    ms_SetConsStep = ESetConstraintStep.SelectLimbType;
                    ms_SetConsStep_Req = ESetConstraintStep.SelectLimbType;
                }
                break;
            case EOPMode.PoseManage:
                {
    ...
    private static void _OnChangeSetConstraintStep(ESetConstraintStep oldStep, ESetConstraintStep newStep)
    {
        ms_SetConsStep = ms_SetConsStep_Req;

        if( newStep == ESetConstraintStep.RotateMidJoint )
        {
            EUtil.ShowNotification("Please_Rotate_To_Bending_Pose\n(Around_90_Degree)", 7.0f);
        }
    }
    ...
    private static void _ManipulateMarkerForIKTarget()
    {
        ...
        if( ms_CurHandleTool == Tool.Move )
        {
            ...
            if (GUI.changed)
            {
                ...
                //SetDirty, Transform should not need this though
                EditorUtility.SetDirty(extTr);
                for (int idx = 0; idx < joints.Length; ++idx)
                {
                    EditorUtility.SetDirty(joints[idx]);
                }
            }
        ...
        else if( ms_CurHandleTool == Tool.Rotate )
        {
            ...
            Handles.BeginGUI();
            {
                ...
                GUILayout.BeginArea(rc);
                {
                    ...
                    if (GUI.changed)
                    {
                        Undo.RecordObject(IKRootJoint, "Rotate IKPlane");
        ...
        else if( ms_CurHandleTool == Tool.RotateIKRoot )
        {
            ...
            if( GUI.changed )
            {
                Undo.RecordObject(IKRootJoint, "IK Root Rotate");
            ...
            case Tool.Move:
                {
                    ...
                    if (EditorGUI.EndChangeCheck())
                    {
                        ...
                        //EditorUtility.SetDirty(selJoint);
                        //ms_bNeedRecalcVerts = true;
                    }
                }
                break;
            case Tool.Rotate:
                {
                    ...
                    //    if (EditorGUI.EndChangeCheck())
                    //    {
                    //        Quaternion deltaRot = newRot * Quaternion.Inverse(rot);
    ...
    private static void _SetVertMaterial()
    {
        ...
        {
            ms_PS.renderer.sharedMaterial = ms_PassThroughMat;
        }
        else if( ms_eShowVertices == EShowVert.NORMAL )
        {
            ms_PS.renderer.sharedMaterial = ms_NormalVertMat;
        }
        else
        {
            ms_PS.renderer.sharedMaterial = ms_BackupMat;
        }
    }
    ...
    private static bool _TryMakeIKLink(ref ISolver isolver, Transform endJoint, int lnkLength)
    {
        ...
        if (isolver is IKLimbSolver)
        {
            ConstraintInfo cinfo;
            if (ms_DictTr2Constraint.TryGetValue(endJoint, out cinfo))
            {
                IKLimbSolver limbSolver = (IKLimbSolver)isolver;
                limbSolver.SetConstraintInfo(cinfo);
            }
            else
            {
                Dbg.LogErr("SMREditor._TryMakeIKLink: the endJoint {0} is not reg-ed in DictTr2Constraint", endJoint);
            }
        }
    ...
    {
        if( ms_DictTr2Constraint.ContainsKey(endJoint) && lnkLen == 2)
        {
            if( inSolver is IKLimbSolver )
            {
                return inSolver;
            }
            else
            {
                return new IKLimbSolver();
            }
        }
        else
        {
            if( inSolver is BaseIKSolver )
            {
                return inSolver;
            }
            else
    ...
    private static void _ApplyPoseToCurrentModel(PoseDataDict data)
    {
        ...
        //Dbg.Log("rootJoint = {0}", rootJoint.name);

        for (var ie = data.GetEnumerator(); ie.MoveNext(); )
        {
            var pr = ie.Current;
            string jointName = pr.Key;
            XformData jointData = pr.Value;

            Transform tr = rootJoint.FindByName(jointName);
            if( tr == null )
            {
                Dbg.LogErr("SMREditor._ApplyPoseToCurrentModel: failed to find joint: {0}", jointName);
                continue;
            }
    ...
    private static void _CreateNewPose(string poseName)
    {
        ...
        ms_PoseSet.AddPose(poseName, ms_SelCtrl.Joints.ToArray());
    ...
    public const string BACKGROUND_TEX = "Assets/Skele/CharacterAnimationTools/Res/black.jpg";
    public const string BACKGROUND_W_TEX = "Assets/Skele/CharacterAnimationTools/Res/white.jpg";
    public const string PassThroughMat = "Assets/Skele/CharacterAnimationTools/Res/DepPass.mat";
    public const string NormalVertMat = "Assets/Skele/CharacterAnimationTools/Res/NormalVertMarker.mat";
    public const string RotateArrowMat = "Assets/Skele/CharacterAnimationTools/Res/RotateArrow.mat";
