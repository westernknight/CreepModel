...
namespace MH
{
    ...
    {
        private List<Transform> m_NeedExportHierarchyNodes = new List<Transform>();
        ...
        /// <summary>
        /// the rootBone must be same with modelImporter's setting, 
        /// we use rootBone as input for RootMotion curves export
        /// </summary>
        public DaeExporter(SkinnedMeshRenderer[] smrs, MeshFilter[] mfs, Transform rootBone)
        {
            m_SMRs = smrs;
            m_MFs = mfs;
            ...
            {
                m_RootBone = rootBone;
                m_AnimRoot = m_RootBone.transform;
                while (m_AnimRoot != null)
                {
                    if (m_AnimRoot.animation != null || m_AnimRoot.GetComponent<Animator>() != null)
                        break;
        ...
        public void Export(AnimationClip clip, string outPath = "export.dae")
        {
            ...
            // library_animations
            _Export_Lib_Animations(col_scene);
        ...
        private void _Export_Lib_Animations(Grendgine_Collada col_scene)
        {
            if (m_Clip == null)
                return;

            var lib_animations = col_scene.Library_Animations = new Grendgine_Collada_Library_Animations();
            List<Grendgine_Collada_Animation> anims = new List<Grendgine_Collada_Animation>();

            float deltaTime = 1f / m_Clip.frameRate;
            float totalTime = m_Clip.length;
            int frameCnt = 0;
            string timeString = _TimeToStringArray(totalTime, deltaTime, out frameCnt);

            //int frameCnt = Mathf.CeilToInt(totalTime * m_Clip.frameRate) + 1; // due to float precision, will be one frame more in some cases

            var curves = AnimationUtility.GetAllCurves(m_Clip, true);
            ...
            foreach (var oneNodeData in animNodesDatum)
            {
                ...
                { // output-source
                    ...
                    matSourceArr.Value_As_String = _AnimNodeToStringArray(oneNodeData, totalTime, deltaTime);
            ...
            lib_animations.Animation = anims.ToArray();
        }
        ...
        private void _Export_Lib_Geometry(Grendgine_Collada col_scene)
        {
            ...
            var geos = lib_geo.Geometry = new Grendgine_Collada_Geometry[meshLst.Count];
            ...
            for (int mIdx = 0; mIdx < meshLst.Count; ++mIdx)
            {
                ...
                nomSrcTechAcc.Count = (uint)m.vertexCount * 3;
        ...
        private void _Export_Lib_VisualScene(Grendgine_Collada col_scene)
        {
            ...
            int nodeCnt = (m_SMRs.Length > 0) ? (m_SMRs.Length + m_MFs.Length + 1) : (m_MFs.Length);
            oneVisualScene.Node = new Grendgine_Collada_Node[nodeCnt];
            ...
            if (m_SMRs.Length > 0)
            {
                ...
                {
                    SkinnedMeshRenderer smr = m_SMRs[idx];
                    if (!m_NeedExportHierarchyNodes.Contains(smr.transform))
                    {
                        var ctrlNode = oneVisualScene.Node[idx] = new Grendgine_Collada_Node();
                        ctrlNode.ID = SCENE_NODE_ID + idx;
                        ctrlNode.Name = smr.name; //SCENE_NODE_ID;
                        ctrlNode.Type = Grendgine_Collada_Node_Type.NODE;

                        ctrlNode.Instance_Controller = new Grendgine_Collada_Instance_Controller[1];
                        var inst_ctrl = ctrlNode.Instance_Controller[0] = new Grendgine_Collada_Instance_Controller();
                        inst_ctrl.URL = "#" + CONTROLLER_ID + idx;
            ...
            if (m_MFs.Length > 0)
            {
                for (int idx = 0; idx < m_MFs.Length; ++idx)
                {
                    ...
                    { //if export this node at the rootNode, not in joints hierarchy
                        ...
                        var geoNode = oneVisualScene.Node[globalIdx] = new Grendgine_Collada_Node();
            ...
            {
                //nodeTr node
                var jointsNode = oneVisualScene.Node[m_SMRs.Length + m_MFs.Length] = new Grendgine_Collada_Node();
                _ExportHierarchyNodes(jointsNode, m_ExtendedJoints[0]);
            }
        ...
        private Matrix4x4 _CalcMatrixFromCurves(AnimationNodeData nodeData, float t)
        {
            ...
            foreach (AnimationClipCurveData curveData in curves) // for each related curve
            {
                ...
                _ApplyCurveDataToTarget(curveData, v, ref pos, ref rot, ref scale);
            }
        ...
        private string _TimeToStringArray(float totalTime, float deltaTime, out int frameCnt)
        {
            frameCnt = 0;
            StringBuilder bld = new StringBuilder();
